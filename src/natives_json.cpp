...
static cell_t WrapChildNode(DataHandle* parent, DataNode* val) {
    if (!val) return 0;
    if (val->refcount >= DataNode::kMaxRefcount) return 0;
    DataHandle* child = new DataHandle(parent->root, val);
    val->refcount++;
    child->owns_refcount_ = true;
    int handle = g_handle_manager.CreateHandle(static_cast<void*>(child), HANDLE_JSON_VALUE);
...
static cell_t Native_JsonArraySetObject(IPluginContext* pContext, const cell_t* params) {
    ...
    if (!child)
        return 0;
    json->ArraySetObject(params[2], child->node);
    return 0;
}
...
static cell_t Native_JsonArrayAppendObject(IPluginContext* pContext, const cell_t* params) {
    ...
    if (!child)
        return 0;
    json->ArrayAppendObject(child->node);
    return 0;
}
...
static cell_t Native_JsonPathGet(IPluginContext* pContext, const cell_t* params) {
    ...
    int path_count = params[0] - 1;
    DataNode* target = ResolveJsonPath(pContext, json->node, params, 2, path_count);
    return WrapChildNode(json, target);
}
...
static cell_t Native_IntMapSetObject(IPluginContext* pContext, const cell_t* params) {
    ...
    if (!child)
        return 0;
    json->IntMapSetObject(key, child->node);
    return 0;
}
